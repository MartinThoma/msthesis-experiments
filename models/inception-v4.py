...
See http://arxiv.org/pdf/1602.07261v1.pdf for details
"""
...
URL = ("https://github.com/titu1994/Inception-v4/releases/download/v1.2/"
       "inception_v4_weights_")
TH_BACKEND_TH_DIM_ORDERING = URL + "th_dim_ordering_th_kernels.h5"
TH_BACKEND_TF_DIM_ORDERING = URL + "tf_dim_ordering_th_kernels.h5"
TF_BACKEND_TF_DIM_ORDERING = URL + "tf_dim_ordering_tf_kernels.h5"
TF_BACKEND_TH_DIM_ORDERING = URL + "th_dim_ordering_tf_kernels.h5"


def conv_block(x, nb_filter, nb_row, nb_col, border_mode='same',
               subsample=(1, 1), bias=False):
    ...
    x = Convolution2D(nb_filter, nb_row, nb_col, subsample=subsample,
...
def create_model(nb_classes=1001, load_weights=True):
    ...
    Returns
    -------
    Keras Model with 1 input and 1 output
    """
    if K.image_dim_ordering() == 'th':
    ...
    if load_weights:
        if K.backend() == "theano":
            if K.image_dim_ordering() == "th":
                weights = get_file(('inception_v4_weights_th_dim_ordering_th_'
                                    'kernels.h5'),
                                   TH_BACKEND_TH_DIM_ORDERING,
                                   cache_subdir='models')
            else:
                weights = get_file('inception_v4_weights_tf_dim_ordering_th_'
                                   'kernels.h5',
                                   TH_BACKEND_TF_DIM_ORDERING,
                                   cache_subdir='models')
        else:
            if K.image_dim_ordering() == "th":
                weights = get_file('inception_v4_weights_th_dim_ordering_tf_'
                                   'kernels.h5',
                                   TF_BACKEND_TH_DIM_ORDERING,
                                   cache_subdir='models')
            else:
                weights = get_file('inception_v4_weights_tf_dim_ordering_tf_'
                                   'kernels.h5',
                                   TH_BACKEND_TF_DIM_ORDERING,
                                   cache_subdir='models')
...
if __name__ == "__main__":
    model = create_model(1000, load_weights=False)
    model.summary()
    from keras.utils import plot_model
    plot_model(model, to_file='inception_v4.png',
