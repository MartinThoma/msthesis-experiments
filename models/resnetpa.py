...
Achieves 92% on cifar-10 according to https://github.com/raghakot/keras-resnet
"""
...
def create_model(nb_classes=10, input_shape=(3, 32, 32),
                 ...
                 init='glorot_normal', reg=0.0, use_shortcuts=True):
    """
    Create ResNet using full pre-activation.
    ...
    nb_classes:
        number of scores to produce from final affine layer (input to softmax)

    layer1_params: tuple of (filter size, num filters, stride for conv)

    res_layer_params: tuple
        of (filter size, num res layer filters, num res stages)

    final_layer_params: None or tuple
        of (filter size, num filters, stride for conv)
