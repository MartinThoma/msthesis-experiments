...
def _calculate_cm_like(model, X, y, n_classes):
    y_i = y.flatten()
...
def create_cm(model_path, data_module, artifacts_path, config):
    ...
    cm = _calculate_cm_like(model, X_train, y_train, n_classes)
    _write_cm(cm, path=os.path.join(artifacts_path, 'cm-like-train.json'))

    # Calculate confusion matrix for test set
    cm = _calculate_cm_like(model, X_test, y_test, n_classes)
    _write_cm(cm, path=os.path.join(artifacts_path, 'cm-like-test.json'))
